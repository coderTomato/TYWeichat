...
#define kSeparatorHeight 6.0f
#define kTitleFontSize 13.0f
#define kButtonTitleFontSize 21.0f
...
@property (copy, nonatomic) ALActionSheetViewDidSelectButtonBlock selectRowBlock;
/** 是否显示*/
...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        self.frame = frame;
        self.cancelTextColor = [UIColor colorWithHex:@"#999999"];
        self.commentTextColor = [UIColor colorWithHex:@"#333333"];
        self.separatorColor = [UIColor colorWithHex:@"#f5f5f9"];
    }
    return self;
}
    ...
    if (self)
    {
        ...
        if ([_otherButtonTitles count] > 0)
        {
            for (int i = 0; i < _otherButtonTitles.count; i++)
            {
                ...
                [btn setTitleColor:self.commentTextColor forState:UIControlStateNormal];
