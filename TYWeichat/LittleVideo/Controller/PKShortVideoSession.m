...
- (void)finishRecording
{
    @synchronized(self) {
        ...
        if (!shouldFinishRecording) return;
    ...
    dispatch_async(_writingQueue, ^{
        @autoreleasepool {
            @synchronized(self) {
                if (self.status != PKSessionStatusFinishingRecordingPart1) return;
...
- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer ofMediaType:(NSString *)mediaType
{
    ...
    dispatch_async(self.writingQueue, ^{
        @autoreleasepool {
            ...
            if (input.readyForMoreMediaData && self.assetWriter)
            {
    ...
    if (newStatus != self.status)
    {
        if ((newStatus == PKSessionStatusFinished) || (newStatus == PKSessionStatusFailed))
        {
            ...
            dispatch_async(self.writingQueue, ^{
                ...
                self.videoInput = nil;
                self.audioInput = nil;
                if (newStatus == PKSessionStatusFailed)
                {//失败删除
